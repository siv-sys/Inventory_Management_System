...
def dashboard():
    ...
    cursor.execute("SELECT categories.title,CAST(SUM(products.quantity) AS SIGNED ) AS total,ROUND((SUM(products.quantity)/(SELECT SUM(P1.quantity) FROM products AS P1 WHERE P1.disabled=0))*100,2) AS percentage FROM products INNER JOIN categories ON products.category = categories.cateid WHERE products.disabled = 0 GROUP BY categories.title ORDER BY CAST(SUM(products.quantity) AS SIGNED ) DESC")
    productByCategory = cursor.fetchall()
    
    cursor.execute("SELECT * FROM products JOIN categories ON products.category=categories.cateid WHERE products.disabled=0 ORDER BY CAST(products.quantity as integer) DESC LIMIT 6")
    recent_products = cursor.fetchall()
    
    cursor.execute("SELECT stocks.stockid,products.productname,products.productcode,users.nickname,SUM(stocks.quantity) AS quantity,stocks.price,stocks.recorddate FROM stocks JOIN users ON stocks.userid = users.userid JOIN products ON products.productid = stocks.productid GROUP BY products.productid, products.productname, products.productcode,stocks.recorddate ORDER BY stocks.recorddate DESC LIMIT 10")
    recent_orders = cursor.fetchall()
    
    cursor.execute("SELECT COUNT(*) AS COUNTER FROM products WHERE products.disabled=0 AND CAST(products.quantity as integer) > 10")
    instock_product = cursor.fetchone()["COUNTER"]
    
    cursor.execute("SELECT COUNT(*) AS COUNTER FROM products WHERE products.disabled=0 AND CAST(products.quantity as integer) > 0 AND CAST(products.quantity as integer) < 10")
    low_stock = cursor.fetchone()["COUNTER"]
    
    cursor.execute("SELECT COUNT(*) AS COUNTER FROM products WHERE products.disabled=0 AND CAST(products.quantity as integer) = 0")
    out_of_stock = cursor.fetchone()["COUNTER"]
    
    total_prices = sum( int(p["quantity"]) * float(p["price"]) for p in products)
    
    cursor.execute("SELECT SUM(CAST(quantity AS integer) * CAST(price AS float)) AS total_price FROM stocks WHERE YEAR(recorddate) = YEAR(NOW())")
    total_orders = cursor.fetchone()["total_price"] or 0
    
    cursor.execute("SELECT SUM(CAST(quantity AS integer) * CAST(price AS float)) AS total_price FROM sales JOIN saledetails ON sales.detailid = saledetails.detailid WHERE YEAR(recorddate) = YEAR(NOW())")
    total_sale = cursor.fetchone()["total_price"] or 0
    
    cursor.execute("SELECT COUNT(*) AS total_customer FROM saledetails WHERE YEAR(recorddate) = YEAR(NOW())")
...
def update_product(id):
    ...
    if request.method == "POST":
        ...
        cursor.execute("UPDATE products SET productcode=%s,productname=%s,category=%s,description=%s,updated_at=%s WHERE productid=%s",(productcode,productname,category,description,updated_at,id,))
...
def show_stock_for_add_stock(id):
    ...
    cursor.execute("SELECT products.*,title FROM products INNER JOIN categories ON products.category = categories.cateid WHERE disabled=0 AND productid=%s",(id,))
    product = cursor.fetchone()
...
def add_stock(id):
    ...
    if request.method == 'POST':
        ...
        if errors:
            ...
            cursor.execute("INSERT INTO stocks(userid,productid,productcode,quantity,price,supplyid,recorddate) VALUES(%s,%s,%s,%s,%s,%s,%s)",(userid,id,productcode,quantity,price_in,supplier_id,recordate))
            conn.commit()
            conn.close()
            inventorys()
...
def inventorys():
    ...
    cursor.execute("SELECT productid,productcode,productname, CAST(quantity as integer) AS quantity,CAST(price as float) AS price,title,products.description,categories.cateid FROM products INNER JOIN categories ON products.category = categories.cateid WHERE disabled=0")
    products = cursor.fetchall()
...
def order_detail():
    ...
    cursor.execute("SELECT stocks.stockid,products.productname,products.productcode,users.nickname,SUM(stocks.quantity) AS quantity,stocks.price,stocks.recorddate FROM stocks JOIN users ON stocks.userid = users.userid JOIN products ON products.productid = stocks.productid GROUP BY products.productid, products.productname, products.productcode,stocks.recorddate ORDER BY stocks.recorddate DESC")
